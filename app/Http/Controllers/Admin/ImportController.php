...
class ImportController extends Controller
{
    ...
    public function startImport(Request $request)
    {
        ...
        if (curl_error($ch)) {
            exit('Unable to connect: ' . curl_errno($ch) . ' - ' . curl_error($ch));
        }
        curl_close($ch);
        ...
        foreach ($jsonData['clients']['client'] as $client) {
            ...
            if ($clientExists) {
                ...
                if (isset($client['address1'])) {
                    $clientExists->address = $client['address1'];
                }
                if (isset($client['address2'])) {
                    $clientExists->address2 = $client['address2'];
                }
                if (isset($client['city'])) {
                    $clientExists->city = $client['city'];
                }
                if (isset($client['state'])) {
                    $clientExists->state = $client['state'];
                }
                if (isset($client['postcode'])) {
                    $clientExists->zip = $client['postcode'];
                }
                if (isset($client['country'])) {
                    $clientExists->country = $client['country'];
                }
                ...
                if (isset($client['address1'])) {
                    $newClient->address = $client['address1'];
                }
                if (isset($client['address2'])) {
                    $newClient->address2 = $client['address2'];
                }
                if (isset($client['city'])) {
