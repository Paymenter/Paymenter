            ...
            <div class="content-box !p-0 overflow-hidden">
                ...
                <table class="w-full">
                    ...
                    <tbody>
                        ...
                        <tr class="{{ $loop->last ? '' : 'border-b-2 border-secondary-200 dark:border-secondary-50' }}">
                            <td class="pl-6 py-3">
                                <div class="flex">
                                    @if ($product->image !== 'null')
                                    <img src="{{ $product->image }}" alt="{{ $product->name }}" class="w-8 h-8 md:w-12 md:h-12 my-auto rounded-md" onerror="removeElement(this);">
                                    @endif
                            ...
                            <td class="py-3">
                                ...
                                <input type="number" name="quantity" wire:change="updateQuantity({{ $product->id }}, $event.target.value)" wire:model="quantity" value="{{ $product->quantity }}" class="w-16 border border-secondary-200 dark:border-secondary-50 dark:bg-secondary-200 rounded-md px-2 py-1 text-sm text-center" min="1" max="100" />
                                @else
                                {{ $product->quantity }}
                                @endif
                            </td>
            ...
            <div class="content-box">
                ...
                <div class="flex flex-row items-start gap-x-2 place-content-stretch">
                    <div class="flex flex-col">
                        <x-input type="text" placeholder="{{ __('Coupon') }}" name="couponCode" wire:model="couponCode" icon="ri-coupon-2-line" class="w-full" />
                    </div>
                ...
                <form wire:submit.prevent="pay">
                    <div class="flex flex-col">
                        ...
                        <select id="payment_method" name="payment_method" autocomplete="payment_method" wire:model="payment_method" class="py-2 bg-secondary-200 text-secondary-800 font-medium rounded-md placeholder-secondary-500 outline-none w-full border focus:ring-2 focus:ring-offset-2 ring-offset-secondary-50 dark:ring-offset-secondary-100 duration-300 border-secondary-300 focus:border-secondary-400 focus:ring-primary-400">
                    ...
                    <div class="items-center p-1">
                        @php
                        $tos = "I agree to the <a href='" . route(' tos') . "' class='text-blue-500 hover:text-blue-600'>terms of service</a>" ; @endphp <x-input id="tos" type="checkbox" name="tos" required :label="$tos" wire:model="tos" />
                    </div>
                    @endif
