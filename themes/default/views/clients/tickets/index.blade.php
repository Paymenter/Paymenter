<script>
    const showBoxView = localStorage.getItem('blockView') == 'true';
    const style = document.createElement('style');

    style.textContent = `
        .ticketBlockMaster { display: ${showBoxView ? 'flex; flex-flow: wrap;' : 'block;'} }
        .ticketBlock { width: ${showBoxView ? '45%' : '92.6666%'} !important; display: ${showBoxView ? 'inline-block' : 'block'}; vertical-align: ${showBoxView ? 'top' : 'middle'}; }
        .ticketBoxRow { background-color: ${showBoxView ? 'transparent' : 'rgb(139 92 246)'} !important; }
        .ticketBoxBox { background-color: ${showBoxView ? 'rgb(139 92 246)' : 'transparent'} !important; }
        ${!document.getElementById('no-ticket-found') ? `
            .ticketBlockMaster { display: block }
        ` : ''}
    `;
    document.head.append(style);

    function showBoxViewSwitch() {
        const blockView = localStorage.getItem('blockView') === 'false' ? 'true' : 'false';
        localStorage.setItem('blockView', blockView);

        style.textContent = `
            .ticketContainer .ticketBlockMaster { display: ${blockView === 'true' ? 'flex; flex-flow: wrap; !important' : 'block; !important'}; }
            .ticketBlock { display: ${blockView === 'true' ? 'inline-block; vertical-align: top; width: 45%;' : 'block; width: 92.6666% !important;'}; }
            .ticketBoxRow { background-color: ${blockView === 'true' ? 'transparent !important;' : 'rgb(139 92 246) !important'}; }
            .ticketBoxBox { background-color: ${blockView === 'true' ? 'rgb(139 92 246) !important;' : 'transparent !important'}; }
        `;
    }
</script>
<x-app-layout>
        ...
        <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="p-6 bg-white border-gray-200 sm:pr-20 sm:pl-12 dark:bg-darkmode2 dark:text-darkmodetext pb-2">
                    <div class="flex flex-row justify-between">
                        <div class="flex flex-row overflow-x-auto lg:flex-wrap lg:space-x-1">
                            ...
                            <div class="flex-none">
                                <button class="dark:text-darkmodetext dark:hover:bg-darkbutton inline-flex w-full justify-center px-2 py-2 font-bold uppercase text-xs p-4 border-b-2 hover:border-violet-300 border-y-transparent text-gray-900 hover:text-violet-300">
                                    {{ __('On Hold') }}
                            ...
                            <button onclick="window.location.href='{{ route('clients.tickets.create') }}'" class="dark:text-darkmodetext dark:bg-darkbutton dark:hover:bg-gray-600 bg-gray-100 hover:bg-gray-600
                                ...
                                <div class="mr-1.5">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                                        ...
                                        <path d="M15.728 9.686l-1.414-1.414L5 17.586V19h1.414l9.314-9.314zm1.414-1.414l1.414-1.414-1.414-1.414-1.414 1.414 1.414 1.414zM7.242 21H3v-4.243L16.435 3.322a1 1 0 0 1 1.414 0l2.829 2.829a1 1 0 0 1 0 1.414L7.243 21z" fill="rgba(165,153,228,1)" />
                                    </svg>
                                </div>
                                {{ __('New Ticket') }}
                            </button>
                            ...
                            <div class="flex flex-row items-center justify-end space-x-2">
                                ...
                                <button class="flex text-base text-gray-700 dark:bg-darkbutton dark:text-darkmodetext dark:hover:bg-gray-600 bg-gray-100 hover:bg-gray-200 rounded-md p-1" onclick="showBoxViewSwitch()">
                                    <div class="ticketBoxBox rounded-md ml-2 p-1">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                                            <path fill="none" d="M0 0h24v24H0z" />
                                            <path d="M3 3h8v8H3V3zm0 10h8v8H3v-8zM13 3h8v8h-8V3zm0 10h8v8h-8v-8zm2-8v4h4V5h-4zm0 10v4h4v-4h-4zM5 5v4h4V5H5zm0 10v4h4v-4H5z" fill="rgba(165,153,228,1)" />
                                        </svg>
                                    </div>
                                    <div class="ticketBoxRow rounded-md ml-2 p-1">
                                        <div>
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24">
                                                <path fill="none" d="M0 0h24v24H0z" />
                                                <path d="M3 4h18v2H3V4zm0 7h18v2H3v-2zm0 7h18v2H3v-2z" fill="rgba(165,153,228,1)" />
                                            </svg>
                                        </div>
                                    </div>
                                </button>
            ...
            <div class="overflow-hidden bg-white shadow-xl sm:rounded-lg dark:bg-darkmode2 mt-6">
                <div class="ticketBlockMaster overflow-hidden bg-white-200 shadow-xl sm:rounded-lg dark:bg-darkmode2 mt-6 pb-2.5">
                    ...
                    @php
                        $tickets = match($sort){
                            'a-z' => $tickets->sortBy('title'),
                            'z-a' => $tickets->sortByDesc('title'),
                            'newest' => $tickets->sortBy('created_at'),
                            'oldest' => $tickets->sortByDesc('created_at'),
                            default => $tickets->sortBy('title'),
                        };
                    @endphp
                            ...
                            <div class="flex flex-row items-start justify-start">
                                <div class="flex flex-col flex-grow w-full space-y-0 self-center text-left">
                                    ...
                                    <div class="text-sm font-medium text-gray-700 dark:text-darkmodetext pt-2 max-w-screen-lg w-full">
                                        <div id='showmore{{$ticket->id}}' style='{!! count(explode("<br />", nl2br($ticketMessages->where("ticket_id", $ticket->id)->last()->message))) > 3 ? "-webkit-mask-image: linear-gradient(black 35%, transparent 100%);" : "" !!}'>
                        ...
                        <div class="flex flex-row items-center justify-between ml-4 pt-4">
                            <div class="flex flex-row items-start justify-start">
                                <div class="shrink-0 w-8 mt-1 mr-4">
                                    <img src="https://www.gravatar.com/avatar/{{ md5(Auth::user()->email) }}?s=200&d=mp" class="h-8 w-full shadow-lg rounded-full ring" onerror='this.error=null;this.src="https://d33wubrfki0l68.cloudfront.net/c0e8a3c6172bd5bebfe787d49974adcff1ec4d3a/ca6a2/img/people/joseph-jolton.png";'>
                                </div>
                                <div class="flex flex-col flex-grow w-full space-y-0 self-center">
                    ...
                    <div class="flex flex-row items-center justify-center">
                        <div class="text-sm font-semibold pt-1" id="no-ticket-found">
                            {{ __('No tickets found.') }}
                        </div>
                    </div>
                    @endif
