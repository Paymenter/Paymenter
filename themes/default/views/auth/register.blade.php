<form
    class="mx-auto flex flex-col gap-4 mt-4 shadow-sm px-6 sm:px-14 pb-10 bg-primary-800 rounded-md xl:max-w-[60%] w-full"
    ...
    <div class="flex flex-col md:grid md:grid-cols-2 gap-4">
        ...
        <x-form.input name="first_name" type="text" :label="__('general.input.first_name')"
            :placeholder="__('general.input.first_name_placeholder')" wire:model.live="first_name" required />
            
        <x-form.input name="last_name" type="text" :label="__('general.input.last_name')"
            :placeholder="__('general.input.last_name_placeholder')" wire:model.live="last_name" required />

        <x-form.input name="email" type="email" :label="__('general.input.email')"
            :placeholder="__('general.input.email_placeholder')" required wire:model.live="email" divClass="col-span-2" />
        ...
        <x-form.input name="password_confirm" type="password" :label="__('Password')"
            :placeholder="__('Confirm your password')" wire:model.live="password_confirmation" required />

        <x-form.properties :custom_properties="$custom_properties" :properties="$properties" />
        
        @if(config('settings.tos'))
            <div class="col-span-2 mt-2">
                <x-form.checkbox name="terms" wire:model.live="terms" required>
                    {!!
                        __('auth.terms_agree_full', [
                            'terms_link' => '<a href="' . config('settings.tos') . '" class="text-secondary-400 hover:underline" target="_blank">' . __('auth.terms_and_policy_link_text') . '</a>'
                        ])
                    !!}
                </x-form.checkbox>
            </div>
    ...
    <x-button.primary class="w-full mt-2" :disabled="$this->isFormInvalid">
        {{ __('Sign up') }}
    </x-button.primary>
