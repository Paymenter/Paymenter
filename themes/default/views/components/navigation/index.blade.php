<nav class="w-full md:px-4 bg-primary-800 md:h-14 flex md:flex-row flex-col justify-between"
        ...
        <div class="flex flex-col md:flex-row">
            ...
            <div class="relative" x-data="{ open: false }">
                <button class="flex flex-row items-center p-3" x-on:click="open = !open">
                    <span class="text-sm @isset($nav['active']) text-secondary @else text-white @endif">{{ $nav['name']
                        }}</span>
                    <!-- arrow down -->
                    <svg xmlns="http://www.w3.org/2000/svg"
                        class="h-4 w-4 ml-2 @isset($nav['active']) text-secondary @else text-white @endif transform "
                        fill="none" x-bind:class="{ 'rotate-180': open }" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="absolute right-0 mt-1 w-48 bg-primary-800 rounded-md shadow-lg z-10 border border-primary-700 "
                    ...
                    <x-navigation.link :href="route($child['route'], $child['params'])"
                        :spa="isset($child['spa']) ? $nav['spa'] : true">{{ $child['name'] }}</x-navigation.link>
                    @endforeach
                </div>
            </div>
            @else
            <x-navigation.link :href="route($nav['route'], $nav['params'] ?? null)"
                :spa="isset($nav['spa']) ? $nav['spa'] : true">{{ $nav['name'] }}</x-navigation.link>
            @endif
            @endforeach
        </div>
        ...
        <div class="flex flex-row mb-2 md:mb-0">
            ...
            <div class="relative">
                ...
                <div class="absolute right-0 mt-2 w-48 bg-primary-800 rounded-md shadow-lg z-10 border border-primary-700 "
                    ...
                    <x-navigation.link :href="route($link['route'], $link['params'] ?? null)"
                        :spa="isset($link['spa']) ? $link['spa'] : true">{{ $link['name'] }}</x-navigation.link>
                    @endforeach
