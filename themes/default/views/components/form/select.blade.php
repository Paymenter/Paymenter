...
<fieldset class="flex flex-col w-full {{ $divClass ?? '' }}">
    ...
    <select id="{{ $id ?? $name }}" {{ $multiple ? 'multiple' : '' }} {{ $attributes->except(['options', 'id', 'name', 'multiple'])->merge(['class' => 'block px-2.5 py-2.5 w-full text-sm text-primary-100 bg-background-secondary border border-neutral
