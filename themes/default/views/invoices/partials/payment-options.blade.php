        ...
        <div>
            ...
            <div class="mb-6">
                ...
                <div wire:click="$set('selectedMethod', 'credit')"
                    ...
                    <div class="flex items-center space-x-4">
                        <div class="text-xl"><x-ri-copper-coin-line class="size-6 text-primary" /></div>
                        <div>
                            <p class="font-medium">Account Credit</p>
                            <p class="text-sm text-base/50">Available: {{ $credit->formattedAmount }}</p>
                        </div>
                    </div>
                    <div class="size-5 rounded-full bg-background-secondary border border-neutral flex items-center justify-center {{ $selectedMethod === 'credit' ? 'border-primary bg-primary' : 'border-neutral-focus' }}">
                        @if($selectedMethod === 'credit') <x-ri-check-line class="size-4 text-white" /> @endif
                    </div>
                </div>
            </div>
            @endif
            ...
            <div class="mb-6">
                ...
                <div class="space-y-3">
                    ...
                    <div wire:click="$set('selectedMethod', '{{ $method->ulid }}')"
                        ...
                        <div class="size-5 rounded-full border border-neutral flex items-center justify-center
            ...
            <div class="mb-6">
                <h3 class="text-lg font-semibold mb-2">{{ __('invoices.pay_with_one_time_method') }}</h3>
                <div class="space-y-3">
                    ...
                    <div wire:click="$set('selectedMethod', 'gateway-{{ $method->id }}')"
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="bg-background-secondary border border-neutral rounded-lg overflow-hidden flex items-center justify-center h-9 w-9">
                                @if($method->extension === 'Stripe')
                                <x-icons.stripe class="size-9" />
                                @elseif($method->extension === 'PayPal')
                        ...
                        <div class="size-5 rounded-full bg-background-secondary border border-neutral flex items-center justify-center {{ $selectedMethod === 'gateway-' . $method->id ? 'border-primary bg-primary' : 'border-neutral-focus' }}">
                            @if($selectedMethod === 'gateway-' . $method->id) <x-ri-check-line class="size-4 text-white" /> @endif
                        </div>
                    </div>
                    @endforeach
                </div>
            ...
            @if($selectedMethod && $selectedMethod !== 'credit' && !str_starts_with($selectedMethod, 'gateway-') && $this->hasRecurringServices)
            <div class="mt-4 p-2">
                <x-form.toggle
                    :label="__('invoices.use_for_recurring')"
