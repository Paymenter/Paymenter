<div class="space-y-4">
    ...
    <a href="{{ route('invoices.show', $invoice) }}" wire:navigate>
        ...
        <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-gray-400 text-sm">{{ $invoice->formattedTotal }}</span>
            </div>
        ...
        @foreach ($invoice->items as $item)
            <p class="text-gray-400 text-sm">Item(s): {{ $item->description }} ({{ __('invoices.invoice_date')}}: {{ $invoice->created_at->format('d M Y') }})</p>
        @endforeach
        </div>
    </a>
    @endforeach
