<!-- Dashboard content -->
<div class="text-lg font-bold py-4">{{ __('dashboard.dashboard_title') }}</div>
<span class="text-md text-base/80 text-nowrap font-semibold mt-4">{{ __('dashboard.welcome_back', ['name' => auth()->user()->name]) }} 👋</span>
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 pb-0 mt-8">
    ...
    <div class="">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="bg-primary flex items-center justify-center font-semibold rounded-md w-5 h-5 text-sm text-white">{{ Auth::user()->services()->where('status', 'active')->count() }}</span>
        </div>
        ...
        <x-navigation.link class="bg-background-secondary hover:bg-background-secondary/80 flex items-center justify-center rounded-lg" :href="route('services')">
    ...
    <div class="">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="bg-primary flex items-center justify-center font-semibold rounded-md w-5 h-5 text-sm text-white">{{ Auth::user()->invoices()->where('status', 'pending')->count() }}</span>
        </div>
        <div class="space-y-4">
            <livewire:invoices.widget :limit="3" />
        </div>
        <x-navigation.link class="bg-background-secondary hover:bg-background-secondary/80 flex items-center justify-center rounded-lg" :href="route('invoices')">
            {{ __('dashboard.view_all') }}
            <x-ri-arrow-right-fill class="w-5 h-5" />
        </x-navigation.link>
    </div>
    ...
    <div class="">
        <div class="flex items-center justify-between mb-6">
            ...
            <span class="bg-primary flex items-center justify-center font-semibold rounded-md w-5 h-5 text-sm text-white">{{ Auth::user()->tickets()->where('status', '!=', 'closed')->count() }}</span>
        </div>
    ...
    <div class="">
        ...
        <x-navigation.link class="bg-background-secondary hover:bg-background-secondary/80 flex items-center justify-center rounded-lg" :href="route('announcements.index')">
            {{ __('dashboard.view_all') }}
            <x-ri-arrow-right-fill class="w-5 h-5" />
        </x-navigation.link>
    </div>
