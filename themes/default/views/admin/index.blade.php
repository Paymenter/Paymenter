        ...
        <div class="py-10 dark:bg-darkmode">
            <div class="mx-auto sm:px-6 lg:px-8">
                <div class="grid p-4 overflow-hidden bg-white shadow-sm dark:bg-darkmode2 sm:rounded-lg md:grid-cols-3">
                    ...
                    <div
                        class="col-span-2 p-10 bg-white border-2 dark:bg-darkmode dark:border-darkmode rounded-xl border-grey-600">
                        <div class="grid grid-cols-3 p-2 pb-10">
                            <div class="p-2 mr-3 rounded-md dark:bg-darkmode2 bg-normal">
                                ...
                                <p class="text-2xl font-bold text-black dark:text-darkmodetext">
                                    {{ App\Models\Orders::whereDate('created_at', '=', date('Y-m-d'))->count() }}</p>
                            </div>
                            <div class="p-2 mr-3 rounded-md dark:bg-darkmode2 bg-normal">
                                <h1 class="text-xl text-gray-500 dark:text-darkmodetext">Tickets today</h1>
                                <p class="text-2xl font-bold text-black dark:text-darkmodetext">
                                    {{ App\Models\Tickets::whereDate('created_at', '=', date('Y-m-d'))->count() }}</p>
                            </div>
                                    ...
                                    <div class="p-2 rounded-md dark:hover:bg-darkbutton dark:bg-darkmode2 bg-normal">
                                        ...
                                        <p class="text-2xl font-bold text-black dark:text-darkmodetext">
                                            ...
                                            @if ($ticket->priority == 'high')
                                                <span
                                                    class="p-1 text-base text-white bg-red-500 rounded-full">High</span>
                                            @elseif($ticket->priority == 'medium')
                                                <span
                                                    class="p-1 text-base text-white bg-yellow-500 rounded-full">Medium</span>
                                            @elseif($ticket->priority == 'low')
                                                <span
                                                    class="p-1 text-base text-white bg-green-500 rounded-full">Low</span>
                                            @endif
                                            @if ($ticket->status == 'closed')
                                                <span
                                                    class="p-1 text-base text-white bg-red-500 rounded-full">closed</span>
                                            @elseif($ticket->status == 'replied')
                                                <span
                                                    class="p-1 text-base text-white bg-yellow-500 rounded-full">replied</span>
                                            @endif
                                        </p>
                                    </div>
                                </a>
