...
<head>
    ...
    @isset($title)
        <meta content="{{ config('app.name', 'TropVPN') }} - {{ $title }}" property="og:title">
        <meta content="{{ config('app.name', 'TropVPN') }} - {{ $title }}" name="title">
    @endisset
    @isset($description)
    ...
    <meta name="theme-color" content="{{ theme('dark-primary', '#4d6cfa') }}">
    {!! hook('head') !!}
</head>
<body
    class="w-full bg-background text-base min-h-screen flex flex-col antialiased"
    x-cloak
    x-data="{
