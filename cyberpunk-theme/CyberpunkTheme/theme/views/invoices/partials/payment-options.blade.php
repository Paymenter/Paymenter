<div>
    ...
    <div class="mb-6">
        ...
        <div wire:click="$set('selectedMethod', 'credit')"
            ...
            class="flex items-center justify-between p-4 cyber-card cyber-clip rounded-lg cursor-pointer transition-all {{ $selectedMethod === 'credit' ? 'border-primary ring-2 ring-primary' : 'border-neutral hover:border-neutral-focus' }}">
            ...
            <div
                class="size-5 rounded-full cyber-card cyber-clip flex items-center justify-center {{ $selectedMethod === 'credit' ? 'border-primary bg-primary' : 'border-neutral-focus' }}">
    ...
    <div class="mb-6" x-data="{ showOneTime: @if($this->savedPaymentMethods->count() == 0) true @else false @endif }">
        ...
        <div class="space-y-3 mt-3" x-show="showOneTime" x-transition>
            ...
            <div wire:click="$set('selectedMethod', 'gateway-{{ $method->id }}')"
                ...
                <div class="flex items-center space-x-4">
                    <div
                        class="cyber-card cyber-clip rounded-lg overflow-hidden flex items-center justify-center h-9 w-9">
                        @if($method->meta?->icon)
                ...
                <div
                    class="size-5 rounded-full cyber-card cyber-clip flex items-center justify-center {{ $selectedMethod === 'gateway-' . $method->id ? 'border-primary bg-primary' : 'border-neutral-focus' }}">
