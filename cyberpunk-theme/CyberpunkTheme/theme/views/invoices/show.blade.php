<div class="container mt-14">
    <div @if ($checkPayment) wire:poll.5s="checkPaymentStatus" @endif>
        ...
        @php $cyberServices = cyber_invoice_services($invoice); @endphp
        @if(count($cyberServices) > 0)
        <div class="cyber-card cyber-clip p-5 mb-4 border-primary/40">
            <div class="flex items-start gap-3">
                <div class="p-2 rounded-lg bg-primary/15 border border-primary/35 shrink-0">
                    <x-ri-instance-fill class="size-5 text-primary" />
                </div>
                <div class="flex-grow">
                    <p class="font-bold">
                        {{ count($cyberServices) === 1 ? 'Esta factura es del servidor:' : 'Esta factura corresponde a estos servidores:' }}
                    </p>
                    <div class="mt-3 flex flex-wrap gap-2">
                        @foreach($cyberServices as $cyberService)
                        <a href="{{ route('services.show', $cyberService['id']) }}" wire:navigate
                            class="inline-flex items-center gap-2 text-sm rounded-lg border border-primary/35 bg-primary/10 px-3 py-1.5 text-primary font-semibold hover:bg-primary/20 transition">
                            <x-ri-server-fill class="size-4" />
                            {{ $cyberService['label'] }}
                            @if($cyberService['product'])
                            <span class="text-base/50 font-normal">· {{ $cyberService['product'] }}</span>
                            @endif
                        </a>
                        @endforeach
                    </div>
                    <p class="text-xs text-base/50 mt-3">
                        Así sabes exactamente a qué servicio pertenece cada pago.
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="cyber-card cyber-clip p-12 rounded-lg mt-2">
