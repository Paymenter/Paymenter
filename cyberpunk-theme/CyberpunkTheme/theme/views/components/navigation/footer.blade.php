@php
$pages = cyber_pages(true);
$socials = cyber_socials();
@endphp
<footer class="w-full px-4 py-4 mt-24 bg-background-secondary/70 border-t border-neutral relative">
    <div class="absolute inset-x-0 top-0 h-px cyber-divider pointer-events-none"></div>
    <div class="container my-10 mx-auto">
        <div class="grid md:grid-cols-3 gap-10">
            <div class="flex flex-col gap-5 items-start">
                ...
                <p class="text-sm text-base/60 max-w-sm">
                    {{ cyber_cfg('footer_text', 'Infraestructura de alto rendimiento para tus webs, bots y aplicaciones.') }}
                </p>
                @if(cyber_bool('socials_enabled', true) && count($socials) > 0)
                <x-cyber.socials :compact="true" />
                @endif
            </div>

            <div class="flex flex-col gap-3">
                <h3 class="text-sm font-bold uppercase tracking-widest text-base/70">Navegación</h3>
                <div class="flex flex-col gap-2 text-sm">
                    <a href="{{ route('home') }}" wire:navigate class="text-base/60 hover:text-primary transition w-fit">{{ __('navigation.home') }}</a>
                    @auth
                    <a href="{{ route('dashboard') }}" wire:navigate class="text-base/60 hover:text-primary transition w-fit">{{ __('navigation.dashboard') }}</a>
                    <a href="{{ route('services') }}" wire:navigate class="text-base/60 hover:text-primary transition w-fit">{{ __('navigation.services') }}</a>
                    <a href="{{ route('invoices') }}" wire:navigate class="text-base/60 hover:text-primary transition w-fit">{{ __('navigation.invoices') }}</a>
                    @endauth
                    @if(!config('settings.tickets_disabled', false))
                    <a href="{{ route('tickets') }}" wire:navigate class="text-base/60 hover:text-primary transition w-fit">{{ __('navigation.tickets') }}</a>
                    @endif
                </div>
            </div>

            <div class="flex flex-col gap-3">
                <h3 class="text-sm font-bold uppercase tracking-widest text-base/70">Información</h3>
                <div class="flex flex-col gap-2 text-sm">
                    @forelse($pages as $page)
                    <a href="{{ url('/p/' . $page['slug']) }}" class="text-base/60 hover:text-primary transition w-fit">{{ $page['title'] }}</a>
                    @empty
                    <span class="text-base/40 text-sm">—</span>
                    @endforelse
                    @if(cyber_ext() && cyber_bool('community_enabled', true))
                    <a href="{{ url('/' . cyber_cfg('community_slug', 'comunidad')) }}" class="text-base/60 hover:text-primary transition w-fit">
                        {{ cyber_cfg('community_name', 'Comunidad') }}
                    </a>
                    @endif
                </div>
            </div>
        ...
        <div class="cyber-divider my-8"></div>
        ...
        <div class="flex flex-col md:flex-row justify-between gap-4 items-center">
            ...
            <a class="border border-neutral p-2 rounded-lg transition-colors group flex items-center gap-2 text-base/45 hover:text-base hover:border-primary/50" href="https://paymenter.org" target="_blank">
