...
<section class="container py-12">
    ...
    <div class="grid gap-4 mt-8 {{ cyber_cols(count($cards)) }}">
        @foreach($cards as $card)
        @php $url = $card['url'] ?? ''; @endphp
        <{{ $url ? 'a' : 'div' }}
            @if($url) href="{{ $url }}" @endif
            class="cyber-card cyber-card-hover cyber-clip p-5 flex flex-col group {{ $align['col'] }} {{ $align['text'] }}">
    ...
    @if(count($features) > 0)
    <div class="grid gap-4 mt-4 {{ cyber_cols(count($features)) }}">
        @foreach($features as $feature)
        <div class="flex items-start gap-3 rounded-xl border border-neutral bg-background-secondary/50 p-4 {{ $align['key'] === 'center' ? 'justify-center' : ($align['key'] === 'right' ? 'justify-end' : '') }}">
