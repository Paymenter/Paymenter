<div class="container mt-14">
    <div class="cyber-card cyber-clip p-6 rounded-lg mt-2">
        ...
        <div class="md:grid grid-cols-4 flex flex-col gap-4">
            <div class="md:col-span-3">
                <div class="flex flex-col gap-4 max-h-[60vh] overflow-y-auto pr-4" wire:poll.10s>
                    ...
                    <div class="cyber-card cyber-card-hover cyber-clip p-4 rounded-lg w-full max-w-[80%]  {{ $message->user_id === $ticket->user_id ? 'ml-auto' : 'mr-auto' }}"
                        @if ($loop->last) x-data x-init="$nextTick(() => $el.scrollIntoView({ block: 'end' }))" @endif>
                        <div class="flex items-center justify-between gap-4">
                            <div class="flex items-center gap-3">
                                <img src="{{ cyber_avatar($message->user) }}" class="size-8 rounded-full border border-neutral bg-background" alt="{{ $message->user->name }} avatar" />
            ...
            <div class="md:order-last order-first w-full col-span-3 sm:col-auto">
                ...
                <div class="flex flex-col w-full col-span-1">
                    <h2 class="text-2xl font-semibold cyber-card cyber-clip p-2 px-4 rounded-md mb-2">
                        {{ __('ticket.ticket_details') }}
                    </h2>
                    <div class="font-semibold flex md:flex-col justify-between cyber-card cyber-clip p-2 px-4 rounded-md gap-2">
                        <h4 class="h-fit">{{ __('ticket.subject') }}:</h4> {{ $ticket->subject }}
                    </div>
                    <div class="font-semibold flex md:flex-col justify-between cyber-card cyber-clip p-2 px-4 rounded-md">
