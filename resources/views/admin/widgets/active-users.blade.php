            ...
            @foreach($sessions as $userSession)
                ...
                <div class="flex justify-between items-center w-full">
                    <div class="flex-1 min-w-0 space-y-1">
                        ...
                        {{-- Username copy thingy --}}
                        <h2 
                            class="font-bold truncate cursor-pointer hover:text-primary-500 transition-colors"
                            x-data="{ copied: false }"
                            x-on:click="
                                navigator.clipboard.writeText('{{ $userSession->user->name }}');
                                copied = true;
                                setTimeout(() => copied = false, 2000);
                            "
                            x-text="copied ? 'Copied!' : '{{ $userSession->user->name }}'"
                        ></h2>
